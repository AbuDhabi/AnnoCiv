    ...
    for (int i=x;i<x+10;i++) {
        for (int j=y;j<y+7;j++) {
            ...
            if (gs.gm.rivers[i][j] == 1) { // danger! array access not sanitized!
                int done=0;
                if (gs.gm.rivers[i-1][j-1] == 1 || gs.gm.tiles[i-1][j-1] == IMG_TILE_WATER) {
                    lineRGBA(MAIN_SCREEN,(i-x)*64,(j-y)*64,(i-x)*64+32,(j-y)*64+32,0,0,150,255); done++;}
                if (gs.gm.rivers[i][j-1] == 1 || gs.gm.tiles[i][j-1] == IMG_TILE_WATER)  {
                ...
                if (gs.gm.rivers[i-1][j] == 1 || gs.gm.tiles[i-1][j] == IMG_TILE_WATER) {
